...
def valid(net, config, test_loader, model_file = None):
    with torch.no_grad():
        ...
        for i in tqdm(range(len(test_loader.dataset)//5)):
            ...
            _data = _data.cuda() #  [5, feature_values, 1024]
            _label = _label.cuda()
        ...
        # print("ap_score:{:.4f}".format(ap_score))
        print("accuracy: {:.4f} | ap_score: {:.4f} | auc: {:.4f}".format(accuracy, ap_score, auc_score))
...
if __name__ == "__main__":
    ...
    if config.modal == "multimodal" and args.missing_modality == True:
        ...
        # Standard (non-multimodal) evaluation
        test_loader = data.DataLoader(
            XDVideo(
                root_dir=config.root_dir,
                mode='Test',
        ...
        valid(net, config, test_loader, model_file=os.path.join(args.model_path, "xd_trans_2022_baseline.pkl"))