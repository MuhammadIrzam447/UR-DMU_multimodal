...
def train(net, normal_loader, abnormal_loader, optimizer, criterion, experiment, index):
    net.train()
    net.flag = "Train"
    ...
    # Log losses to Comet ML
    for key, value in loss.items():
        if isinstance(value, torch.Tensor):
            value = value.item()  # Convert tensor to scalar
        experiment.log_metric(f"train_loss_{key}", value, step=index)

    # # Compute and log training accuracy
    # accuracy = (predict.argmax(dim=1) == _label).float().mean().item()
    # experiment.log_metric("train_accuracy", accuracy, step=index)
