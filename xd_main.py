...
if __name__ == "__main__":
    ...
    if config.modal == "multimodal":
        config.len_feature = 1024 + 128
    else:
    ...
    # Set the logging level for Comet to WARNING to suppress INFO logs
    ...
    # exit()
    test(net, config, experiment, test_loader, test_info, 0)
    for step in tqdm(
        ...
        train(net, normal_loader_iter,abnormal_loader_iter, optimizer, criterion, experiment, step)
        if step % 10 == 0 and step > 10:
            test(net, config, experiment, test_loader, test_info, step)
            if test_info["ap"][-1] > best_auc:
                ...
                torch.save(net.state_dict(), os.path.join(args.model_path, f"{experiment_name}_xd_trans_{config.seed}.pkl"))
